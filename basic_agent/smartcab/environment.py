...
class Environment(object):
    ...
    # For  the purpose of not to stuck in  deadlocks in case of enforce_deadline is False, 
    # end trial when deadline reaches 'end_trial_time' value,  which is decreased at each trial.    
    end_trial_time = -100  
#my add
    start = 0
    destination = 0
    ...
    def step(self):
        ...
        for agent in self.agent_states.iterkeys():
            agent.update(self.t)
#MY ADD : Return exits the current function or method. 
#Pass is a null operation and allows execution to continue at the next statement.
        ...
        if self.primary_agent is not None:
            ...
            if agent_deadline <= self.end_trial_time:
                self.done = True
                print "Environment.step(): when agent come across end_trial_time  ({})! Trial stop !".format(self.end_trial_time)
    ...
    def act(self, agent, action):
        ...
        if agent is self.primary_agent:
            if state['location'] == state['destination']:
                ...
                result_deadline = self.agent_states[self.primary_agent]['deadline']
...
## csv file create           
            result = "{},{},{},{},{},{},{},{},{},{}\n".format(start,destination,inputs_oncoming,inputs_left,inputs_right, light, action,heading,location,reward)		
            with open("forcsv.csv", "a") as myfile:
                    myfile.write(result)  
